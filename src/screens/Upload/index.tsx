...
export function Upload() {
  ...
  async function handleUpload(){
    ...
    // reference.putFile(image).then(() => Alert.alert('Upload concluído!')).catch((error) => console.log(error))
    const uploadTask = reference.putFile(image);
    uploadTask.on('state_changed', taskSnapshot => {
      const percent = ((taskSnapshot.bytesTransferred / taskSnapshot.totalBytes) * 100).toFixed(0);
      setProgress(percent);

      setBytesTransferred(`${taskSnapshot.bytesTransferred}bytes transferidos de ${taskSnapshot.totalBytes}bytes`);
    })
